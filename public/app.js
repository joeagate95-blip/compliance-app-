...
function statusClass(s){
  return s==='Expired'?'red':s==='Due Soon'?'amber':'green';
}

function fmt(d){
  return d?new Date(d).toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'}):'Not set';
}
    ...
    <div class="login">
      <div class="login-card">
        ...
        <form id="login">
          <div class="field">
            <label>Email</label>
            <input name="email" value="landlord@demo.co.uk">
          </div>
          <div class="field">
    ...
    <div class="shell">
      <aside class="side">
        ...
        ${nav.filter(n=>n!=='admin'||state.user.role==='administrator').map(n=>`
          <a href="#" class="${state.view===n?'active':''}" data-view="${n}">
            ${title(n)}
          </a>
        `).join('')}
...
function allItems(){
  return [
    ...
    ...state.data.properties.map(p=>({
      id:p.id,
      propertyId:p.id,
      item:'Property Condition Review',
      title:'Next Review',
      exp:p.nextConditionReview,
      type:'review'
    }))
  ];
}
  ...
  return `
    <div class="grid">
      <button class="card metric-card span3" onclick="state.view='properties';render()">
        <div class="metric">${props.length}</div>
        <b>Properties</b>
      ...
      <button class="card metric-card span3" onclick="state.view='expiry';state.filter='due';render()">
        <div class="metric amber">${due}</div>
        <b>Due Soon</b>
        <span>Open due list</span>
      </button>

      <button class="card metric-card span3" onclick="state.view='expiry';state.filter='expired';render()">
        <div class="metric red">${overdue}</div>
        <b>Expired</b>
        <span>Open expired list</span>
      </button>
      ...
      <div class="card span4">
        <h2>Quick Links</h2>
        <p><button onclick="openContractorLink()">Create Contractor Upload Link</button></p>
        <p><button class="btn2" onclick="openTenantLink()">Create Tenant Maintenance Link</button></p>

        <h2>Reminders</h2>
        ${state.data.reminders.slice(0,5).map(r=>`
          <p>
            <span class="pill">${new Date(r.at).toLocaleDateString('en-GB')}</span><br>
            ${r.message}
          </p>
        `).join('')||'<p class="muted">No reminders logged yet.</p>'}
      </div>
    </div>
  `;
}
    ...
    <table>
      ...
      ${props.map(p=>`
        <tr>
          ...
          <td>
            <button class="btn2" onclick="openCompliance('${p.id}')">
      ...
      ${state.data.properties.map(p=>{
        ...
        return `
          <div class="card span6">
            ...
            <button class="btn2" onclick="openContractorLink('${p.id}')">Contractor Upload Link</button>
...
function compliance(){
  ...
  const props=state.selected
    ? state.data.properties.filter(p=>p.id===state.selected)
    ...
    <div class="grid">
      ${props.map(p=>`
        <div class="card span12">
          <button class="btn2" onclick="state.view='properties';state.selected=null;render()">
            Back to Portfolio
          </button>
            ...
            ${cats.map(c=>{
              ...
              return `
                <div class="card span4">
                  ...
                  <button class="btn2" onclick="state.view='documents';state.selected='${p.id}';state.filter='${c}';render()">
                    View Documents
                  </button>
                </div>
              `;
...
function documents(){
  ...
  return `
    <div class="tabs">
      ...
      ${state.data.properties.map(p=>`
        <button onclick="state.selected='${p.id}';state.filter=null;render()">
          ${p.address.split(',')[0]}
        </button>
      `).join('')}
    </div>
...
function contractors(){
  return `
    <div class="grid">
      <div class="card span12">
        <h2>Contractor Upload Links</h2>
        <p class="muted">Send a property-specific link to a contractor so they can upload completed compliance documents directly.</p>
        <button onclick="openContractorLink()">Create Contractor Upload Link</button>
      </div>
      ...
      ${state.data.reviews.map(r=>{
        ...
        return `
          <div class="card">
            ...
            <p>
              <b>Date:</b> ${fmt(r.date)}
              | <b>Completed by:</b> ${r.completedBy}
      ...
      <form id="reviewForm" enctype="multipart/form-data">
        <div class="field">
          <select name="propertyId">
            ${state.data.properties.map(p=>`<option value="${p.id}">${p.address}</option>`)}
          </select>
        </div>
        ...
        <div class="field">
          <select name="outcome">
            ...
            <option>Minor Issues Identified</option>
  ...
  return `
    <div class="grid">
      <div class="card span4">
        ...
        <p>Connect OCR to read CP12/EICR dates automatically.</p>
      </div>
      <div class="card span4">
        <h2>Contractor Scheduling</h2>
        <p>Book contractors against properties and compliance categories.</p>
      </div>
      <div class="card span4">
        <h2>AI Compliance Risk Checker</h2>
        <p>Future module to flag missing or high-risk compliance areas.</p>
      </div>
    </div>
  `;
}
    ...
    <div class="grid">
      ...
      <div class="card span12">
        ...
        ${state.data.audit.map(a=>`
          <p>
            <span class="pill">${new Date(a.at).toLocaleString('en-GB')}</span>
            ${a.action}
  ...
  modal(`
    ...
    <form id="docForm" enctype="multipart/form-data">
      <div class="field">
        <select name="propertyId">
          ${state.data.properties.map(p=>`
            <option value="${p.id}" ${p.id===propertyId?'selected':''}>${p.address}</option>
          `)}
        </select>
      </div>
